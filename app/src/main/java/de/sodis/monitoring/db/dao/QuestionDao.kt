...
interface QuestionDao {
    ...
    fun getQuestionItems(surveyId: Int): List<QuestionItem>

    @Query("SELECT * FROM Question q LEFT JOIN SurveySection ss on ss.id = q.surveySectionId LEFT JOIN SurveyHeader sh ON sh.id = ss.surveyHeaderId WHERE sh.id = :surveyId")
    fun getQuestions(surveyId: Int): List<Question>
